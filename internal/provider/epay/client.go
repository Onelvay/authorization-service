...
func (c *Client) getToken(requestSrc Request) (responseSrc Token, err error) {
	...
	resBytes, status, err := c.handler("POST", path, writer.FormDataContentType(), "", reqBytes)
	if err != nil {
		return
	}
...
func (c *Client) handler(method, path, contentType, accessToken string, reqBytes io.Reader) (resBytes []byte, status int, err error) {
	// setup request
